...
import { jobsStore, proposalsStore, generateUuid } from '../../../lib/apiStore';
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!topic || topic.length < 5) {
    return res.status(400).json({ detail: 'Topic must be at least 5 characters' });
  }
  ...
  proposalsStore[jobId] = {
    ...
    sections: [
      ...
      { title: '2. Literature Review', content: 'A thorough review of existing literature reveals several important themes.' },
